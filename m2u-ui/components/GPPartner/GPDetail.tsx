...
import { BASIC_STATUS, gender, gp_qualification, language, qualification, STATUS } from '@/utils/constents'
...
import type { GPDetail } from './GPnterface'
...
export default function GPDetail({
  ...
  const handleReview = (newStatus: string) => {
    reviewGPPartner({ adminApprovalStatus: newStatus })
    setStatus(newStatus)
  }
  ...
  // Submit remark and execute reviewGP
      ...
      >
        <div className="px-6 py-6 sm:py-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <p className="text-xl sm:text-2xl font-bold">GP Details</p>
          ...
          {/* Details Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-x-8 sm:gap-y-6 mt-4">
            ...
            <TextCointainer Icon={ClipboardDocumentListIcon} label="Reg Number" value={data?.registerNumber} />
            <TextCointainer
            ...
            <TextCointainer
              ...
              value={`${findFromOptions(gp_qualification, data.medicalQualification)} ${data.medicalQualification === 'OTHERS' ? `(${data.other})` : ''}`}
            />
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">
                {actionType === BASIC_STATUS.ACTIVE ? 'Activate' : 'Deactivate'} G.P Partner
              </h3>
